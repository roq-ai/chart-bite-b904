...
import { getCommodityChartById, updateCommodityChartById } from 'apiSdk/commodity-charts';
...
function CommodityChartEditPage() {
  ...
  const handleSubmit = async (values: CommodityChartInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateCommodityChartById(id, values);
      mutate(updated);
      resetForm();
